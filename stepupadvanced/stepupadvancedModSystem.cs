...
namespace stepupadvanced
{
    public class StepUpAdvancedModSystem : ModSystem
    {
        ...
        {
            if (player?.Entity?.World == null) return false;
            ...
            foreach (var pos in positionsToCheck)
            {
                ...
                var codeLocation = block.Code;
                if (codeLocation == null) continue;

                string code = codeLocation.ToString();

                if (serverList.Contains(code)) return true;
                if (clientList.Contains(code)) return true;
            }
